...
from src.main.composer.balance_editor_composer import balance_edit_composer
...
def edit_balance(user_id):
    try:
        # Valida JWT + UID do header
        token_information = auth_jwt_verify()

        # Cria HttpRequest para o caso de uso
        ...
        # Executa caso de uso
        http_response = balance_edit_composer().handle(http_request)
